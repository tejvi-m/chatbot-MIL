...
def task():
    ...
    natural_language_understanding=NaturalLanguageUnderstandingV1(version='2018-11-16',iam_apikey='xx',url='https://gateway-lon.watsonplatform.net/natural-language-understanding/api')
...
def txt2spch(text):
    ...
    os.system("t2s.mp3")
'''
def intersectlist(user_input, wiki):
    count=0
    wikilist_1=nlp_sent(wiki)
    wikilist=[]
    #print(wikilist)
    for sent in wikilist_1.sents:
        print(sent)
        sent=filter_line(sent, whitelist)
        wikilist.append(sent)
    final_wiki_list=[]
    for i in wiki:
        for j in user_input:
            if i==j:
                count+=1
        if(count>=2):
            final_wiki_list.append(i)
            wiki='.'.join(final_wiki_list)
            return wiki
        else:
            final_wiki_list=wikilist[:2]
            wiki='.'.join(final_wiki_list)
            return wiki
'''
    ...
    def resp():
        while(1):
            ...
            if user_input in list_of_extended_ques:
                ...
                df_response=detect_intent_texts('xx','unique',user_input,'en')
                if "joke" in df_response:
                    ...
                    if user_input in no_response:
                        ...
                        df_response=detect_intent_texts('chatbot-oydwtm','unique',user_input,'en')
                    print(">",df_response)
                    #txt2spch(df_response)
                # elif "game" in df_response:
                #     print(">",df_response)
                #     user_input=input("Enter query: ")
                #     user_input=filter_line(user_input,whitelist)
                #     if user_input in game_request:
                #         print("Okay, Which game do you want to play?")
                #         print("1. Minesweeper")
                #         print("2. Escapido")
                #         user_input=input("Enter choice: ")
                #         if user_input=="1":
                #             os.chdir("C:/Users/taran/OneDrive/Desktop/chatbot-MIL/games/minesweeper")
                #             os.system("python ./main.py 1")
                #             os.chdir("C:/Users/taran/OneDrive/Desktop/chatbot-MIL")
                #             print("Good game eh?")
                #         elif user_input=="2":
                #             os.system("python ./games/escapido/Level_2.py 1")
                #             print("Had a good game?")
                #         else:
